/*
    ...
    --------------------------------------------------------------------------
    13/02/16 Stephen L. Smith
    Additional functions added to facilitate the use of a validation set with runCGP() and repeatCGP():

    DLL_EXPORT struct chromosome* runValiCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, int numGens);

    DLL_EXPORT struct results* repeatValiCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, int numGens, int numRuns);
    --------------------------------------------------------------------------
*/
...
/*
	SLS - repetitively applies runValiCGP() to obtain average behaviour with validtion set
*/
DLL_EXPORT struct results* repeatValiCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, int numGens, int numRuns){
	...
	for(i=0; i<numRuns; i++){
		...
		/* run cgp */
		rels->bestChromosomes[i] = runValiCGP(params, data, valiData, numGens);
...
/*
	SLS - runValiCGP() - version of runCGP() with validtion set
*/
struct chromosome* runValiCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, int numGens){

	int i;
	int gen;

	/* bestChromo found using runValiCGP */
	struct chromosome *bestChromo;

	/* SLS validChromo used for calculating validation fitness of bestChromo */
	struct chromosome *valiChromo;
	...
	if(params->updateFrequency != 0){
		printf("\n-- Starting CGP --\n\n");
		printf("Gen\tFitness\t\tValidation fitness\n");
	}
	...
	for(gen=0; gen <numGens; gen++){
		...
		/* SLS Initialise validation chromosome (and copy) from best chromosome */
		valiChromo = initialiseChromosomeFromChromosome(bestChromo);

		/* SLS reset the fitness of the validation chromosome */
		valiChromo->fitness = -1;

		/* SLS Set (calculate) validation chromosome */
		setChromosomeFitness(params, valiChromo, valiData);  /* SLS validation fitness */

		/* check termination conditions */
/*		if(getChromosomeFitness(bestChromo) <= params->targetFitness){

			if(params->updateFrequency != 0){
				printf("%d\t%f\t%f - Solution Found\n", gen, bestChromo->fitness, valiChromo->fitness);
			}
...
*/
		/* display progress to the user at the update frequency specified */
		if(params->updateFrequency != 0 && (gen % params->updateFrequency == 0 || gen >= numGens-1) ){
			//printf("%d\t%5.2f\t%5.2f\n", gen, bestChromo->fitness, valiChromo->fitness); /* SLS validation fitness */
			printf("%d\t%5.2f\t%5.2f\n", gen, 100-bestChromo->fitness*100, 100-valiChromo->fitness*100); /* SLS validation fitness */
		}
...
/*
	SLS - repetitively applies runValiTestCGP() to obtain average behaviour with validtion and test sets
*/
DLL_EXPORT struct results* repeatValiTestCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, struct dataSet *testData, int numGens, int numRuns){
	...
	for(i=0; i<numRuns; i++){
		...
		/* run cgp */
		rels->bestChromosomes[i] = runValiTestCGP(params, data, valiData, testData, numGens);
...
/*
	SLS - runValiTestCGP() - version of runCGP() with validtion and test sets
*/
struct chromosome* runValiTestCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, struct dataSet *testData, int numGens){

	int i;
	int gen;

	/* bestChromo found using runValiTestCGP */
	struct chromosome *bestChromo;

	/* SLS validChromo used for calculating validation fitness of bestChromo */
	struct chromosome *valiChromo;

	/* SLS testChromo used for calculating test fitness of bestChromo */
	struct chromosome *testChromo;
	...
	for(gen=0; gen <numGens; gen++){
		...
		/* SLS Initialise test chromosome (and copy) from best chromosome */
		testChromo = initialiseChromosomeFromChromosome(bestChromo);

		/* SLS reset the fitness of the test chromosome */
		testChromo->fitness = -1;

		/* SLS Set (calculate) test chromosome */
		setChromosomeFitness(params, testChromo, testData);  /* SLS test fitness */

		/* check termination conditions */
/*		if(getChromosomeFitness(bestChromo) <= params->targetFitness){

			if(params->updateFrequency != 0){
				printf("%d\t%f\t%f\t%f - Solution Found\n", gen, bestChromo->fitness, , valiChromo->fitness);
			}
...
struct chromosome* fRunValiTestCGP(struct parameters *params, struct dataSet *data, struct dataSet *valiData, struct dataSet *testData, int numGens,int arrayIndex, double fitnessArray[3][numGens]){
...
*/
		...
		if(params->updateFrequency != 0 && (gen % params->updateFrequency == 0 || gen >= numGens-1) ){
			printf("%d\t%5.2f\t\t%5.2f\t\t\t%5.2f\n", gen, 100-bestChromo->fitness*100, 100-valiChromo->fitness*100, 100-testChromo->fitness*100); /* SLS validation and test fitness */
            fitnessArray[0][arrayIndex] = 100-bestChromo->fitness*100;
            fitnessArray[1][arrayIndex] = 100-valiChromo->fitness*100;
            fitnessArray[2][arrayIndex] = 100-testChromo->fitness*100;
            arrayIndex++;
			//printf("%d\t%5.2f\t%5.2f\t%5.2f\n", gen, bestChromo->fitness, valiChromo->fitness, testChromo->fitness); /* SLS validation and test fitness */
		}
...
/*
	...
	SLS - now returns the average error
*/
static double supervisedLearning(struct parameters *params, struct chromosome *chromo, struct dataSet *data){
	...
	return (error/getNumChromosomeOutputs(chromo))/getNumDataSetSamples(data);
}
